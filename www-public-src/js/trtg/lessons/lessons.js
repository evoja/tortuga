...
var givCurrentVersion = ns_get("trtg.lessons.ParamsUtil.givCurrentVersion");
var getLesson = ns_get("trtg.lessons.ParamsUtil.getLesson");
var htmlspecialchars = ns_get("om.text.htmlspecialchars");
...
var getVersionLesson = function()
{
    ...
    if (position != -1)
    {
        var version = document.URL.slice(position + 1, position + 1 + givCurrentVersion().length);
        return version;
    }
    else
    {
...
ns.initLessons = function(bg, list, descrDiv, env, allContainers)
{
    ...
    var CURRENT_VERSION = givCurrentVersion();
